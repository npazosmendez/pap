...
ull bishops(ull i, ull l, ull k){
    ...
    if (i==n/2) return k==1 ? l : 0; // llegue a la "columna" del medio, donde puedo poner un único alfil en "l" lugares
    return
        // como miro el tablero girado 45gr, en cada nueva "columna" se agregan 2 espacios libres
        bishops(i+1, l+2-1, k-1) * l * 2 + // pongo 1 acá. Tengo "l" formas. Como miro la mitad del tablero, tengo el doble de formas
        bishops(i+1, l+2-2, k-2) * (l*l-l)/2 * 2+ // pongo 2 acá. Tengo comb(l 2) formas. Como miro la mitad del tablero, tengo el doble de formas
        bishops(i+1, l+2, k); // no pongo ninguna acá
    
}

ull bishops(ull k) {return bishops(1, 2, k);} 
